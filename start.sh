...
# the gitpod user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the gitpod user to the desired the user (NB_USER)
    if id gitpod &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" gitpod 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the gitpod user:"
            _log "- username: gitpod       -> ${NB_USER}"
            _log "- home dir: /home/gitpod -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the gitpod user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the gitpod home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "gitpod" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/gitpod to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/gitpod/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/gitpod to /home/${NB_USER}!"
                _log "Attempting to symlink /home/gitpod to /home/${NB_USER}..."
                if ln -s /home/gitpod "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/gitpod to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/gitpod/"* ]]; then
...
# user we started as.
    ...
    gitpod_UID="$(id -u gitpod 2>/dev/null)"  # The default UID for the gitpod user
    gitpod_GID="$(id -g gitpod 2>/dev/null)"  # The default GID for the gitpod user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old gitpod user to nayvoj ($(id -u gitpod):$(id -g gitpod))"
            ...
            sed --expression="s/^gitpod:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/gitpod:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "gitpod" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/gitpod. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/gitpod ]]; then
        _log "WARNING: no write access to /home/gitpod. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
